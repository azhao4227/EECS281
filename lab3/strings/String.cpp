...
String& String::erase(size_t pos, size_t len) {
    ...
    char* first = &cstr[pos];
    char* second = &cstr[pos+len];

    if(pos + len >= sz){
        cstr[pos] = a_null_byte;
        sz = pos;
    } else {
        while(*second != a_null_byte){
            *first = *second;
            first++;
            second++;
        }
...
String& String::insert(size_t pos, const String& str) {
    ...
    size_t midIndex = sz-1;
    size_t moveIndex = sz + use.sz-1;
    for(size_t i = 0; i < sz-pos+1; i++){      
        cstr[moveIndex] = cstr[midIndex];
        moveIndex--;
        midIndex--;
    }

    cstr[sz+use.sz] = a_null_byte;

    for(size_t i = 0; i < use.sz; i++){      
        cstr[pos+i] = use[i];
    }
