...
cost best_price(const std::vector<cost>& prices) {
    ...
    std::vector<std::vector<cost>> memo(size, std::vector<cost>(6,std::numeric_limits<cost>::max()/4));
    int k = 1;
    ...
    for(size_t i = 1; i < size; i++){
        for(int j = 0; j < 6; j++){
            cost addPrice = prices[k];
            if(j == 0){
                memo[i][0] = std::min(memo[i-1][0] + discounted(addPrice), memo[i-1][5]);
            } else {
                memo[i][j] = std::min(memo[i-1][j] + discounted(addPrice), memo[i-1][j-1] + addPrice);
            }
        }
        k++;

    }
    return memo[size - 1][0];
}
